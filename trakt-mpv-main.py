...
fh = logging.FileHandler("/tmp/trakt.log")
...
def query(flags, configs):
    ...
    # Custom match to ignore whatsapp videos
    if re.search(r'(vid-\d+-wa\d+.*)', media, re.IGNORECASE):
        sys.exit(14)
...
def __query_search_ep(name, season, ep, configs):
    LOG.info("Searching episode " + name + " " + season + "-" + ep)
    """ Get the episode """
    c_name = name.replace('.', ' ')
    c_name = c_name.replace('_', ' ').strip()
    ...
    # Re checkeamos para usar el anio
    if len(res.json()) == 0:
        res = requests.get(
            'https://api.trakt.tv/search/show',
